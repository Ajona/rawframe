...
export default function Checkout() {
  ...
  // Item passed via navigation state or fallback demo item
  const item = location.state?.item || {
    id: 1,
    title: 'Nairobi Marathon Start Line',
    price: 4.99,
    type: 'RAW',
    tag: '#NairobiMarathon',
    creator: 'Jane Doe',
  ...
  const handlePaymentSuccess = async ({ method, fields }) => {
    setShowPay(false);

    try {
      // Initiate payment via backend
      const { data: initiated } = await paymentsAPI.initiate({
        type: 'content',
      ...
      // Redirect to payment provider (Flutterwave or similar)
      if (initiated.paymentLink) {
        window.location.href = initiated.paymentLink;
      } else {
        setDone(true);
      }
    } catch (err) {
