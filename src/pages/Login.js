  ...
  return (
    <div style={{ minHeight:'100vh', background:C.cream, display:'flex',
      ...
      <div style={{ background:'#fff', border:`1px solid ${C.border}`, borderRadius:8,
        ...
        <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:'1.6rem', fontWeight:700, marginBottom:6 }}>Welcome back</h2>
        <p style={{ color:C.muted, fontSize:'0.875rem', marginBottom:'1.75rem' }}>Sign in to your RawFrame account</p>

        {/* Google */}
        <GoogleAuthButton onSuccess={handleGoogle} label="Sign in with Google" />
        ...
        {error && (
          <div style={{ background:'#fce8e8', color:C.danger, padding:'0.7rem 1rem',
            borderRadius:4, fontSize:'0.85rem', marginBottom:'1rem',
            border:`1px solid rgba(184,76,46,0.2)` }}>{error}</div>
        )}

        <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
          {[{ k:'email', label:'Email', type:'email', ph:'you@example.com' },
            { k:'password', label:'Password', type:'password', ph:'••••••••' }].map(f => (
            <div key={f.k}>
          ...
          <button onClick={handleSubmit} disabled={loading}
            style={{ width:'100%', padding:'0.85rem', background: loading ? C.muted : C.ink,
              color:C.cream, border:'none', borderRadius:4, fontSize:'0.9rem', fontWeight:500,
        ...
        <p style={{ textAlign:'center', fontSize:'0.85rem', color:C.muted, marginTop:'1.5rem' }}>
          ...
          <span onClick={() => navigate('/signup')}
            style={{ color:C.gold, cursor:'pointer', fontWeight:500 }}>Create one free</span>
        </p>
      </div>
    </div>
  );
